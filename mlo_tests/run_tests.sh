#!/bin/bash
# Verification suite for pair_style lj/cut/mlo.
# Usage: mlo_tests/run_tests.sh [build-dir]      (default: build-new)
# Run from the lammps-stable_22Jul2025 directory.
LMP="./${1:-build-new}/lmp"
[ -x "$LMP" ] || { echo "no binary at $LMP"; exit 1; }
T=mlo_tests
pass=0; fail=0
ok()   { printf '  \033[32mPASS\033[0m  %s\n' "$1"; pass=$((pass+1)); }
bad()  { printf '  \033[31mFAIL\033[0m  %s   %s\n' "$1" "$2"; fail=$((fail+1)); }

run() { $LMP -log none -in "$1" "${@:2}" 2>&1; }
...
ref=""; consistent=1
for nt in on off; do for np in 1 2 4; do
  v=$(mpirun --oversubscribe -np $np $LMP -log none -in $T/mpi_consistency.in -var nt $nt 2>&1 \
      | grep '^S0' | sed 's/.*sfz=\([^ ]*\).*/\1/')
  [ -z "$ref" ] && ref="$v"
  awk "BEGIN{d=$v-$ref; if(d<0)d=-d; exit !(d/$ref < 1e-12)}" || consistent=0
done; done
[ $consistent = 1 ] && ok "sum fz identical across np=1,2,4 x newton on/off" \
