    ...
    id_temp(nullptr), random(nullptr)
{
  ...
  // t_period is the `damp` parameter!
  t_period = utils::numeric(FLERR, arg[5], false, lmp);
  aniso_ratio = utils::numeric(FLERR, arg[6], false, lmp);
...
void FixLangevinMLO::setup(int vflag)
{
...
void FixLangevinMLO::post_force(int /*vflag*/)
{
...
void FixLangevinMLO::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixLangevinMLO::post_force_templated()
{
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      fran[2] = sqrt(aniso_ratio)*gamma2*(random->uniform()-0.5);

      if (Tp_BIAS) {
        temperature->remove_bias(i,v[i]);
        fdrag[0] = gamma1*v[i][0];
        fdrag[1] = gamma1*v[i][1];
        fdrag[2] = aniso_ratio*gamma1*v[i][2];
...
void FixLangevinMLO::compute_target()
{
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixLangevinMLO::end_of_step()
{
  double **v = atom->v;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixLangevinMLO::reset_target(double t_new)
{
...
int FixLangevinMLO::modify_param(int narg, char **arg)
{
...
double FixLangevinMLO::compute_scalar()
{
...
void *FixLangevinMLO::extract(const char *str, int &dim)
{
  dim = 0;
